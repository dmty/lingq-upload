...
`probe_chapters` delegates to `codecs::SymphoniaMetadata`, which reads the MP4 boxes directly via `codecs::mp4_chapters::read_chapters`: a QuickTime chapter track (`tref/chap` → the referenced text `trak`) if present, otherwise a Nero `chpl` box. A container with neither yields an empty vec. Parsing runs on a `spawn_blocking` thread.

**Contract surface:** atom times are converted to f64 seconds at parse time — QuickTime durations against the chapter track's own `mdhd` timescale, `chpl` timestamps against its fixed 100 ns unit. Callers never see raw units, so timescale variance between sources (`1/1000` on typical encoder output, `1/44100` on Audible delivery) is invisible above `codecs/`. Atom titles come from the text-track samples (QuickTime) or the `chpl` entry strings (Nero).

**Empty result is normal.** Population D (no atoms) returns `Ok(vec![])`. The caller treats this as "whole file is one track."

**Probe failure degrades to whole-file.** A malformed or unreadable container returns `AudioError::Probe` / `AudioError::Io`; `core::job::expand_single_file` logs a `tracing::warn!` with the path and error and treats the file as one track. Fewer than two surviving atoms takes the same whole-file path.

## Filter rules

`core::audio::filter_atoms(atoms, total_duration_sec)` filters the raw list before the matcher sees it. `core::job::expand_single_file` applies it between the probe and the fan-out, using `probe_duration` for the total.

1. **Duration threshold.** Drop atoms whose duration is below `max(6.0, total_duration / atom_count / 10)` seconds. The dynamic term catches tiny intro / branding atoms (observed: a 41 s "第1章" preamble in a 21,961 s file where every real chapter is > 2,000 s; dynamic threshold ≈ 244 s drops it). On real-world audiobooks (≥ 6 atoms × ≥ 600 s) the dynamic term is always > 60 s and the floor is a no-op. The floor only bites on very short files (synthetic fixtures, short Drama-CDs where some real atoms are ~200 s) and is set to 6.0 s so it filters obvious sub-10 s branding atoms without consuming legitimate short atoms. A failed duration probe leaves the threshold on its floor rather than skipping the filter.
2. **No re-ordering, no merging.** Atoms come out of the parser in file order; that order is the contract. Adjacent atoms with the same title are not merged (they may represent intentional sub-sections).

Atom coverage is deliberately *not* checked against file duration. A trailing gap between the last atom's `end` and the file end is common float drift (0.014 s observed in a 56,402 s file) and never changes which atoms are usable, so there is nothing for the user to act on.
...
- `window: None` → decode the whole stream and encode it.
- `window: Some((start, end))` → `AudioDecoder::seek(start)`, then feed the encoder at most `(end − start) × sample_rate` frames, truncating the final frame to the exact boundary.

Duration verify per slice: `|(end − start) − probed_mp3_duration| < 1.0 s` → `AudioError::DurationMismatch`, same as the whole-file path.
...
| `synth_chapters_intro.m4b` | 3 | 120 s | First atom is 5 s; the duration threshold drops it, leaving 2. Models the tiny-intro case |

The fixtures are committed, so neither the build nor CI needs the generator; it requires a local ffmpeg only when the fixtures are regenerated. Its muxer normalises chapter timescales to `1/1000` on output, so the fixtures cannot exercise the `1/44100` timescale directly — acceptable because timescale conversion happens inside `mp4_chapters` and the structural path under test is identical. A real-world Audible m4b remains a useful manual smoke test if locally available.
