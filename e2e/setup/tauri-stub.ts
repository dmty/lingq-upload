...
type StubArgs = Record<string, any> | undefined;

// Runs in the page, not in the test process: it is serialized by
// page.addInitScript, so it must not reference anything outside its own body.
export function tauriStub(workerIndex: number): void {
  ...
  const WORKER_NS = String(workerIndex);
  ...
  const TRANSCRIPTION_PREFERENCES_KEY =
  ...
  window.__pickerState__ = {
    ...
    chaptersByProject:
      (window.__pickerState__ && window.__pickerState__.chaptersByProject) ||
      {},
  ...
  window.__TAURI_EVENT_PLUGIN_INTERNALS__ = {
    unregisterListener(event, id) {
      ...
      window.__eventHandlers__[event] = ids.filter(
        (candidate) => candidate !== id,
      );
      delete callbacks["_" + id];
    },
  };
  ...
  window.__emitEvent__ = (event, payload) => {
    const ids = window.__eventHandlers__[event] || [];
    for (const id of ids) {
      const cb = callbacks["_" + id];
      if (cb) cb({ event, id, payload });
    }
  };

  const handlers: Record<string, (args: StubArgs) => unknown> = {
    ...
    cmd_project_load: (args) => {
      ...
      const meta =
        (window.__projectMeta__ && window.__projectMeta__[key]) || {};
      return {
        schema_version: 1,
        id: {
          content_hash: key,
    ...
    cmd_apply_mapping_op: (args) => {
      ...
      if (op.kind === "swap") {
        const idx = next.pairs.findIndex(
          (p: any) => p.chapter_id === op.chapter_id,
        );
        ...
        const srcIdx = next.pairs.findIndex(
          (p: any) => p.track_id === op.track_id,
        );
        ...
        if (displaced && displaced !== op.track_id) {
          ...
          if (!next.parking_lot.includes(displaced))
            next.parking_lot.push(displaced);
        }
        ...
        next.pairs[idx].touched = true;
        next.parking_lot = next.parking_lot || [];
        if (!next.parking_lot.includes(op.track_id))
          next.parking_lot.push(op.track_id);
    ...
    cmd_confirm_mapping: (args) => {
      ...
      if (args) {
        window.__confirmMappingCalls__.push(
          args as NonNullable<typeof window.__confirmMappingCalls__>[number],
        );
      }
      return null;
    },
    ...
    cmd_cancel_job: (args) => {
      ...
      if (args) {
        window.__cancelJobCalls__.push(
          args as NonNullable<typeof window.__cancelJobCalls__>[number],
        );
      }
      return null;
    },
    ...
    upload_one_shot: async () => {
      ...
      return (
        window.__uploadOneShotResult__ ?? { lesson_id: 101, collection_id: 7 }
      );
    },
    ...
    cmd_set_transcription_preferences: (args) => {
      if (window.__failNextTranscriptionPreferences__) {
        ...
        throw {
          kind: "Other",
          message: "Could not save transcription preferences.",
        };
      }
    ...
    cmd_detection_availability: (args) => {
      ...
      const base = byProject[key] ||
        window.__detectionAvailability__ || {
          eligible: false,
          condition: null,
          chapter_count: 0,
          track_count: 0,
      ...
      const evidence =
        readDecisions()[key]?.detection ?? base.existing_evidence ?? null;
      return {
        ...base,
        ...
        can_start:
          base.eligible && evidence == null && keyPresent && consentMatches,
      };
    },
    cmd_accept_transcribe_consent: async (args) => {
      window.__transcribeConsentCalls__ =
        window.__transcribeConsentCalls__ || [];
      if (args) {
        window.__transcribeConsentCalls__.push(
          args as NonNullable<typeof window.__transcribeConsentCalls__>[number],
        );
      }
      if (window.__failNextTranscribeConsent__) {
        ...
        throw {
          kind: "Other",
          message: "Could not save transcription consent.",
        };
      }
    ...
    cmd_detect_start_offset: async (args) => {
      ...
      if (args) {
        window.__detectionStartArgs__.push(
          args as NonNullable<typeof window.__detectionStartArgs__>[number],
        );
      }
      window.__detectionListenerCountAtStart__ = (
        window.__eventHandlers__.job || []
      ).length;
      if (window.__detectionGate__) await window.__detectionGate__;
      if (window.__detectionCommandError__)
        throw window.__detectionCommandError__;
      return (
        window.__detectionResult__ || {
          kind: "no_transcript",
          reason: "content_poor",
        }
      );
    ...
    cmd_confirm_detected_range: async (args) => {
      window.__confirmDetectedRangeCalls__ =
        window.__confirmDetectedRangeCalls__ || [];
      if (args) {
        window.__confirmDetectedRangeCalls__.push(
          args as NonNullable<
            typeof window.__confirmDetectedRangeCalls__
          >[number],
        );
      }
      ...
      const chapters = window.__pickerState__.chaptersByProject[key] || [];
      const start = chapters.findIndex(
        (chapter) => chapter.id === range.start_chapter_id,
      );
      const end = chapters.findIndex(
        (chapter) => chapter.id === range.end_chapter_id,
      );
    ...
    cmd_reset_detection: async (args) => {
      ...
      if (args) {
        window.__resetDetectionCalls__.push(
          args as NonNullable<typeof window.__resetDetectionCalls__>[number],
        );
      }
    ...
    "plugin:event|listen": (args) => {
      ...
      (window.__eventHandlers__[ev] = window.__eventHandlers__[ev] || []).push(
        id,
      );
      return id;
    },
